...
1. READ: The `Array.prototype.forEach` function takes a function which iterates over the array allowing you to do something with each element. It is one of the example of using the Array.prototype functions to not need to write a for loop on your own - an passing a call back function to process the array.  

    Now with ES6 notation, we can use an arrow function. View the following example used in the /Demos section. Notice how a variable is set for sport, to represent the single sport being processed, each time the supplied function is called - working against the `sportsArray.` 
...
1. Now, you will use the `forEach` method with your array. Create a `logHobbies` function with no inputs which uses the `forEach` method for looping, and prints a message using ES6 backticks with string template references for the properties. 
...
1. READ: we want to execute the babel command to turn our ES6 code into ES5. 
    ...
    The `npx` command will look in the node_modules directory, if nto there, it looks globally, if not there it will temporarly download the program, execute it, then delete it.
...
1. If it is at least 5.0 you can use npx. If not, you can update npm it to the latest version using ```npm i -g npm```

1. Issue this command to compile the entire src directory and output the contents to a /dist directory:
    ```
   npx babel src --presets babel-preset-env --out-dir dist
    ```

1. Look in the `/dist` directory at the transpiled file. It should not be using ES5 type concatenation instead of backticks and not use the arrow function.
...
1. If you have more time, you can look at the docs on `package.json` https://docs.npmjs.com/getting-started/using-a-package.json
