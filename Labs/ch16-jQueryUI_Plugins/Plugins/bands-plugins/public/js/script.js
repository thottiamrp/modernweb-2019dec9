...
function showBands(jsonData) {
    ...
    // Start html output
    var out = `<h1>Bands</h1>
               <table class="full-width"><tr><th>Name</th><th>Origin City</th><th>Origin Country</th><th>Year Formed</th><th>Genres</th><th>Action</th></tr>`;
    ...
    for (var i = 0; i < bands.length; i++) {
        out += '<tr><td>' + bands[i].name + '</td><td>' + bands[i].city +
            ...
            '<button type="submit" id="delete" onclick="deleteBand(' + bands[i].id + ')">Delete</button> ' +
            '</div></td></tr>';
    }
